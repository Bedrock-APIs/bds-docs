...
export class Block { private constructor(); readonly dimension: Dimension; readonly location: Vector3; readonly permutation: BlockPermutation; readonly x: number; readonly y: number; readonly z: number; getComponent(componentName: string): BlockComponent; setPermutation(permutation: BlockPermutation): void}
...
export class Entity { private constructor(); readonly dimension: Dimension; readonly id: string; readonly location: Vector3; nameTag: string; readonly typeId: string; addEffect(effectType: EffectType | string, duration: number, options?: EntityEffectOptions): void; addTag(tag: string): number; applyDamage(amount: number, options?: EntityApplyDamageByProjectileOptions | EntityApplyDamageOptions): number; applyImpulse(vector: Vector3): void; applyKnockback(directionX: number, directionZ: number, horizontalStrength: number, verticalStrength: number): void; clearVelocity(): void; getComponent(componentId: string): EntityComponent; getComponents(): EntityComponent[]; getEffect(effectType: EffectType | string): Effect; getEffects(): Effect[]; getHeadLocation(): Vector3; getTags(): string[]; getVelocity(): Vector3; getViewDirection(): Vector3; hasComponent(componentId: string): number; hasTag(tag: string): number; kill(): number; removeEffect(effectType: EffectType | string): number; removeTag(tag: string): number; runCommand(commandString: string): CommandResult; runCommandAsync(commandString: string): Promise<CommandResult>; teleport(location: Vector3, teleportOptions?: TeleportOptions): void; tryTeleport(location: Vector3, teleportOptions?: TeleportOptions): number}
//@ts-ignore allow class inheritance for native classes
...
export class ItemStack { constructor(itemType: ItemType | string, amount?: number); readonly amount: number; readonly isStackable: number; readonly keepOnDeath: number; readonly lockMode: ItemLockMode; readonly maxAmount: number; readonly nameTag?: string; readonly 'type': ItemType; readonly typeId: string; getComponent(componentId: string): ItemComponent; getComponents(): ItemComponent[]; hasComponent(componentId: string): number; isStackableWith(itemStack: ItemStack): number}
